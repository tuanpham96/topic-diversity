...
```{r}
# test_file <- sprintf('data/nonblock-models/data-%06d.rds', 431)
test_file <- sprintf('data/block-models/data-%06d.rds', 58)
td <- readRDS(test_file)
info <- td$info
info
```
...
analysis_func <- analyze_diversity
analysis_args <- list(nrep_robustness = 5)

anly_div_time <- analysis_through_time(td, 
                                       samp_time = samp_time,
                                       analysis_function = analysis_func,
                                       analysis_arguments = analysis_args)
