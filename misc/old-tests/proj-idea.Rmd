...
mdl_gen=list(
  agent = partial(sample_pa,power=2.3,m=1,directed=F),
  topic = partial(sample_pa,power=2.5,m=1,directed=F))


mdl_gen=list(
  agent = partial(barabasi.game,power=1,directed=F),
  topic = partial(barabasi.game,power=1,directed=F))

# mdl_gen=list(
#   agent = partial(sample_pa,power=1,m=1,directed=F),
#   topic = partial(sample_pa,power=0.5,m=1,directed=F))


# mdl_gen=list(
#   agent = partial(sample_gnp,p=0.1,directed=F),
#   topic = partial(sample_gnp,p=0.01,directed=F))

# mdl_gen=list(
#   agent = partial(sample_pa,power=1,m=1,directed=F),
#   topic = partial(sample_gnp,p=0.05,directed=F))

# mdl_gen=list(
#   agent = partial(sample_sbm,pref.matrix=cbind(c(.1, .005), c(.005, .1)),
#                   block.sizes=c(25,25),directed=F),
#   topic =  partial(sample_sbm,pref.matrix=cbind(c(.05, .001), c(.001, .05)),
#                   block.sizes=c(100,100),directed=F))
...
init_layout <- layout_with_kk(G, weights=ifelse(crossing(pseudo_cl, G),10,1))
V(G)$x <- init_layout[,1]
V(G)$y <- init_layout[,2]

df_vertices <- as_data_frame(G, 'vertices')
df_edges <- as_data_frame(G, 'edges')
df_edges <- mutate(df_edges, terminus.censored = TRUE, onset=0)
  ...
  for (ni in 1:max_nsteps) {
    ...
    if (length(edge_seq) > 0) {
      new_topics_df <- mutate(new_topics_df, onset = ni, terminus.censored = TRUE)
      df_edges <- bind_rows(df_edges, new_topics_df)
...
nodename2nodeid <- function(node_df, node_name) {
  node_df %>% filter(name == node_name) %>% select(node_id) %>% as.numeric()
}
nodelist_name2id <- function(L, node_df) {
  lapply(L, function(.) {nodename2nodeid(node_df, .)}) %>% unlist
}

df_edges$terminus <- max_nsteps
df_edges$head_nodeid <- nodelist_name2id(df_edges$from, df_vertices)
df_edges$tail_nodeid <- nodelist_name2id(df_edges$to, df_vertices)

df_edges <- select(df_edges, c('onset','terminus','tail_nodeid', 'head_nodeid',
                               'terminus.censored', 'color')) %>%
  mutate(duration = terminus - onset)
df_vertices <- df_vertices %>% select(node_id, color, x, y)%>%
  mutate(onset=0, terminus=max_nsteps)

thenetwork <- network(
  df_edges %>% mutate(from = head_nodeid, to = tail_nodeid) %>% select(from,to),
  vertex.attr = df_vertices,
  directed = FALSE,
  bipartite = FALSE
)
...
compute.animation(
  ...
  layout.par = list(x='x',y='y',color='color'),
  slice.par = list(
    start = 0,
    end = max_nsteps,
    interval = 1,
...
# layout <- layout_with_fr(G, weights=ifelse(crossing(pseudo_cl, G),1,1000))

# svg("figures/demo/demo-net.svg", bg = "transparent")
png("figures/demo/demo-net.png", bg = "transparent",  width = 1600, height = 1600)
plot(G, layout=layout, 
     vertex.label=NA,
     vertex.size=5, vertex.frame.color = rgb(0,0,0,0), 
     edge.curved=0.05, edge.width=6)
...
# p_alpha_vec <- c(0.1,0.3,0.5, 0.6, 0.7, 0.8, 0.9, 0.95)
...
topic_entropy_dfs <- mclapply(1:num_trials, function(ind_trial) {
  topic_entropy_df <- lapply(p_alpha_vec, function(p_alpha) {
    p_beta <- 1 - p_alpha
    G <- initialize_unipartite(n_a, n_t, tau_0, mdl_gen = mdl_gen)
    G_new <-
      update_bipartite_topicagent(G,
                                  max_nsteps,
                                  p_alpha,
                                  p_beta,
                                  tau_max,
...
```{r}
z_err <- 1
c('num_topics_agg', 'topic_entropy') %>% lapply(function(name_of_Y) {
  topic_entropy_dfs %>%
    mutate(Y = !!sym(name_of_Y), p_a = factor(alpha)) %>%
    group_by(t, p_a) %>%
    summarize(
      mean = mean(Y),
      sd   = sd(Y),
