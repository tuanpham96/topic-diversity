...
```{r for testing}
td <- readRDS(file.path(data_path, 'data-000691.RData'))

topic_graph <- induced_subgraph(td$G,filter_noderole(V(td$G), 'topics')$name)
agent_graph <- induced_subgraph(td$G,filter_noderole(V(td$G), 'agents')$name)

image(as_adj(td$G, sparse=F))
image(as_adj(topic_graph, sparse=F))
image(as_adj(agent_graph, sparse=F))

plot(degree_distribution(topic_graph))

tau_max <- general_params$tau_max
df_edges_selected <- as_data_frame(td$G) %>% 
  filter(onset <= tau_max & substr(from, start = 1, stop = 1) != substr(to, start = 1, stop = 1))

summarized_by_topics <- select(df_edges_selected, to) %>% group_by(to) %>% summarise(count = length(to)) 

summarized_by_topics %>% 
...
```{r}
num_blocks <- 10 
num_topics <- general_params$n_t
num_agents <- general_params$n_a

topic_block_groups <- tibble(
  topic_node = 1:num_topics,
  topic_group = rep(1:num_blocks, rep(num_topics/num_blocks,num_blocks)),
  topic_name = paste("t", 1:num_topics, sep = "") 
)

topic_block_groups
# ggplot(topic_block_groups, aes(topic_node, topic_group)) + 
#   geom_line()
...
anly_fields <- c('topic_entropy', 'group_topic_entropy', 'num_topics', 'sub_dist_median')

sapply(uni_models, function(mdl_name) {
  select_anly <- time_analysis %>% 
    filter(model_topic == mdl_name,
           model_agent == 'sbm_3')
...
ggarrange(plotlist = g_list,
          ncol = length(anly_fields),
          nrow = length(uni_models),
          common.legend = T)
